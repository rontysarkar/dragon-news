...
const HomeNewsCard = ({ aNews }) => {
    const { title, thumbnail_url, author, details, category_id } = aNews

    return (
        <div className="flex flex-col  p-6 space-y-6 overflow-hidden rounded-lg shadow-md dark:bg-gray-50 ">
            ...
            <div>
                ...
                <div className="text-sm dark:text-gray-600">{
                    details.length > 200 ? <h1>{details.slice(0, 200)}<Link to={`/news/${category_id}`} className='text-blue-500' > ReadMore.......</Link></h1> : details
                }</div>
            </div>
